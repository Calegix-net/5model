...
weight_dir = 'weight_pth_file_random(10c10r)'
summary_dir = 'summary_results_random(10c10r)'
...
# main_random.pyを実行し、all_layers_summary.csvを収集する
def run_main_and_collect(run_id):
    ...
    for attempt in range(MAX_RETRIES):
        ...
        try:
            # main_random.pyを実行
            subprocess.run(['python', 'main_random.py'], check=True)
            ...
            print(f"Error: main_random.py の実行に失敗しました (試行 {attempt + 1}/{MAX_RETRIES})")
            time.sleep(10)  # 少し待機してから再試行
            
    print(f"Error: main_random.py の実行に {MAX_RETRIES} 回失敗しました。次の実行に進みます。")
    return None
...
def create_final_dataset():
    ...
    # 指定回数 main_random.py を実行し、データを収集
