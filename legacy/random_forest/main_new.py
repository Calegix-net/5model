...
# 'Attack_Type'のユニークな値を表示
print("Unique Attack_Type values before mapping:")
...
# 関数を適用
middle_layers_df = middle_layers_df.groupby(['Run_ID']).apply(calculate_change_rate).reset_index(drop=True)
final_layers_df = final_layers_df.groupby(['Run_ID']).apply(calculate_change_rate).reset_index(drop=True)
