...
summary_directory = os.path.join(parent_directory, 'summary_results_random(10c10r)')
...
# データをロードする関数
def load_data(partition_id, random=False):
    ...
    # 悪意のあるデータ変更
    if random:
        partition_train_test = apply_random_label_change(partition_train_test, "train")
        partition_train_test = apply_random_label_change(partition_train_test, "test")
        
    data_collator = DataCollatorWithPadding(tokenizer=tokenizer)
    trainloader = DataLoader(partition_train_test["train"], batch_size=8, collate_fn=data_collator, shuffle=True)
    testloader = DataLoader(partition_train_test["test"], batch_size=8, collate_fn=data_collator)
    return trainloader, testloader

# 悪意のあるラベル変更関数
def apply_random_label_change(dataset, split: str):
    for idx in range(len(dataset[split])):
        if np.random.rand() < MALICIOUS_DATA_RATIO:
            dataset[split][idx]["labels"] = (dataset[split][idx]["labels"] + 1) % 2  # ラベル反転
    return dataset
...
def client_fn(cid: str) -> fl.client.Client:
    random = (int(cid)+1) / NUM_CLIENTS <= MALICIOUS_NODE_RATIO  # 悪意のあるクライアントかどうかを判定
    model = AutoModelForSequenceClassification.from_pretrained(MODEL_NAME, num_labels=2).to(DEVICE)
    trainloader, testloader = load_data(int(cid), random=random)
...
fl.simulation.start_simulation(
    ...
    client_resources={"num_cpus": 3, "num_gpus": 0.25}
)
